...
public class CreateObject extends SetupUtils {
	...
	public void createNewObject() throws InterruptedException {
		...
		// Thread.sleep(5000);
		// WebElement UploadImg = dr.findElement(By.xpath("//input[@type =
		// 'file']"));
		 ...
		 * 
		 * Pages.objectPage().materialsListBox.sendKeys("acrylic paint"
		 * +Keys.ENTER); Pages.objectPage().heightTextEdit.sendKeys("111");
		 * Select height_dropdown = new
		 * Select(Pages.objectPage().heightDropdown);
		 * height_dropdown.selectByVisibleText("cm");
		 * Pages.objectPage().widthTextEdit.sendKeys("111"); Select
		 * width_dropdown = new Select(Pages.objectPage().widthDropdown);
		 * width_dropdown.selectByVisibleText("mm");
		 * Pages.objectPage().depthTextEdit.sendKeys("111"); Select
		 * depth_dropdown = new Select(Pages.objectPage().depthDropdown);
		 * depth_dropdown.selectByVisibleText("cm");
		 * Pages.objectPage().lengthTextEdit.sendKeys("111"); Select
		 * length_dropdown = new Select(Pages.objectPage().lengthDropdown);
		 * length_dropdown.selectByVisibleText("cm");
		 * 
		 * Pages.objectPage().diameterTextEdit.sendKeys("111"); Select
		 * diameter_dropdown = new Select(Pages.objectPage().diameterDropdown);
		 * diameter_dropdown.selectByVisibleText("cm");
		 * 
		 * Pages.objectPage().weightTextEdit.sendKeys("111"); Select
		 * weight_dropdown = new Select(Pages.objectPage().weightDropdown);
		 ...
		 * //----------Research section--------------
		 * 
		 * Pages.objectPage().yearTextEdit.sendKeys("1800"); Select
		 * date_unit_dropdown = new Select(Pages.objectPage().unitDropdown);
		 * date_unit_dropdown.selectByValue("AD"); Thread.sleep(2000);
		 */
	...
	public void editObject() throws InterruptedException {
		...
		/*
		 * dr.navigate().refresh();
		 * Pages.consignmentPage().sothebysLogo.click(); waitforElement(dr,
		 * Pages.objectPage().navItemObject);
		...
		// Assert.assertEquals(Pages.objectPage().creatorValue.getText(),
		// creator_edit);
